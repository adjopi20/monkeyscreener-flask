...
@info_bp.route('/api/info/<symbol>', methods=['GET'])
def get_info(symbol):
    try:
...
@info_bp.route('/api/info2/<symbol>', methods=['GET'])
...
@info_bp.route('/api/info/stocklist2', methods=['GET']) #cadangan seandainya sumber data yang dr cache tidak tersedia
def get_all_info2():
    stock_arr = []
    symbol_arr = addJK()
...
@info_bp.route('/api/info/stocklist', methods=['GET'])
def get_all_info():
    stock_info = {}
    stocks_info = []
...
@info_bp.route('/api/clear_cache/<key>', methods=['POST'])
def clear_cache(key):
    try:
...
@info_bp.route('/api/hist/<sector>/<metric>/hist.png')
